...
void main() {
  group('UC-11: ExpenseService.createExpense input validation', () {
    ...
    test(
      'keeps the pre-generated expense identifier through persistence',
      () async {
  ...
  group('UC-11: ExpenseService.createExpense membership validation', () {
    test(
      'throws ExpensePayerNotMemberException when payer is not a member',
      () async {
        final service = ExpenseService(
  ...
  group('UC-12: ExpenseService.updateExpense AF-01 and persistence', () {
    test(
      'splits 10.00 across three participants and keeps identity fields',
      () async {
  ...
  group('UC-14: ExpenseService.getDebtsForUser input validation', () {
    final service = ExpenseService();

    test('rejects empty wgId', () {
      expect(
        () => service.getDebtsForUser(wgId: '', userId: 'user-1'),
        throwsA(isA<ArgumentError>()),
      );
    });

    test('rejects whitespace-only wgId', () {
      expect(
        () => service.getDebtsForUser(wgId: '   ', userId: 'user-1'),
        throwsA(isA<ArgumentError>()),
      );
